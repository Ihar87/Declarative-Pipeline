//pipeline to run codequality. All jenkins job parameters are in hybris_module.groovy file
...
node(env.NODE_NAME) {
  ...
  try{
    ...
    stage("Copy test results to WORKSPACE") {
        sh """
            mv ${HYBRIS_ROOT}/hybris/log/checkstyle ${WORKSPACE}/log/
            mv ${HYBRIS_ROOT}/hybris/log/pmd ${WORKSPACE}/log/
        """
    }
    ...
    stage("Analyze results") {
        def pmdCounter = "2100"
        checkstyle canComputeNew: false, defaultEncoding: '', failedTotalAll: '0', healthy: '', pattern: 'log/checkstyle/checkstyle_*.xml', unHealthy: '', unstableTotalAll: '0'
        pmd pattern: "log/pmd/*.xml", failedTotalHigh: "${pmdCounter}", usePreviousBuildAsReference: true
    }
  } catch(err) {
    throw err
    currentBuild.result = "FAILURE"
    error "CodeQuality tests failed"
  } finally {
    //drop created schema
